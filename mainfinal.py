...
def user_login():
    ...
    #if username:
    #    success_label.config(text=f"Welcome, {username}!", fg="green")
    #else:
    #    success_label.config(text="Please enter a username.", fg="red")
...
def posturedetect():
    ...
    usertypename = user_login()
    
    success_label.config(text=f"Welcome, {usertypename}!", fg="green")
    print(usertypename)
    if(usertypename):
        cameraison = False
        time.sleep(2)   
            ...
            while True:
                  ...
                  if distance < 100:
                      ...
                      if cameraison == True:
                          ...
                          print("Body Posture: ", body_posture)
                          ...
                          if (distance >= min_eye_distance or distance <= max_eye_distance) and body_posture == "Correct Posture":
                              print("Within Good Eye Range and HAS GOOD POSTURE")
                              continue
                              ...
                              while(body_posture == "Incorrect Posture"):
                                  ...
                                  timeCount += 1
                                  time.sleep(1)
                                  print("Time Stipulated: ", timeCount)
                                  if timeCount == 10:
                                      print("Time has Reached 10 Seconds! Taking Screenshot.. ")
                              ...
                              if(body_posture != "Incorrect Posture" and bp_status == 1):
                                  print("Details of Incorrect Posture Saved into Database.")
...
def main():
    ...
    start_session_button = tk.Button(window, text="Start Session", command=posturedetect, font=("Arial", 12), bg="#4CAF50", fg="white")
    start_session_button.pack(pady=10, ipadx=10, ipady=5)
    
    # Stop Session button
    stop_session_button = tk.Button(window, text="Stop Session", command=stop_detection_loop, font=("Arial", 12), bg="#FF5733", fg="white")
    stop_session_button.pack(pady=10, ipadx=10, ipady=5)


    # Label to display success/failure message
    success_label = tk.Label(window, text="", font=("Arial", 12))
    success_label.pack()
