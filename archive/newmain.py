...
def user_login():
    """Displays login form and returns username."""
    st.title("User Login")
    name = st.text_input("Enter your name")
    if st.button("Start Session"):
        if name:
            st.success(f"Welcome, {name}!")
            return name  # Return username for subsequent use
        else:
...
def switch_on_camera(max_retries=3, retry_interval=1):
    """Attempts to open the camera, handles retries."""
...
def analyze_posture(cap):
    """Analyzes posture and returns feedback."""
    ...
    frame.release()  # Release the frame
